...
class Board():
    ...
    def find_children(self):
        ...
        for action in allActions:
            obj, hassh = self.make_move(action)
            allObjs.append(obj)
            allHash.append(hassh)
 
        return allObjs, allHash
    ...
    while(gameOver==False):
        for u in b.unitList:
            if u ==u2: # the player we are intertsed to learn its behaviour! (Ranged unit for now, and the behaviour is "attack closeset" for now!)
                ...
                print(bestAction)
                if isinstance(bestAction, Board):
                    print(bestAction.p1_actions)
                    print(bestAction.p2_actions)
                    b.p1_actions = bestAction.p1_actions
                    b.p2_actions = bestAction.p2_actions
                    b.Draw()
                    u.actionList = b.p2_actions
                    
                else:
                    u.actionList.append(list(bestAction[3])) #p2_action
                ...
                if choice[:6] =="attack":
                    unitName = choice[7:]
                    # targetUnit = b.returnUnitbyName(b.unitList, unitName)
                    u.actionList.append([choice[:6], unitName])
                else:
                    u.actionList.append([choice, None])
