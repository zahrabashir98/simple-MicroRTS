...
_TTTB = namedtuple("equivalentBoard", "tup turn winner terminal")
...
def start(x1, y1, x2, y2,x3, y3, flag):
    b = Board()
    b.Draw()
    ...
    while(gameOver==False):
        for u in b.unitList:
            if u ==u2: # the player we are intertsed to learn its behaviour! (Ranged unit for now, and the behaviour is "attack closeset" for now!)
                ...
                if choice[:6] =="attack":
                    ...
                    u.actionList.append([choice[:6], targetUnit])
                else:
        ...
        for key in tree.children:
            for againEach in tree.children[key]:
                print(againEach.p1_actions)
                print(againEach.p2_actions)
