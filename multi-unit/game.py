...
class Board():
    ...
    def hash(self): #TODO: CHANGE FOR multi-units game
        ...
        for each in self.unitList:
            hit_list.append(each.hitpoints)

        return (tuple(self.board), self.nodeType, self.single_unit_action, tuple(p1), tuple(p2), tuple(hit_list), self.winner, self.is_over)
    ...
    def find_children(self, unitName): #TODO: if ownerID= 2 has more than one unit, then we should specify the name
        ...
        dictionary = {}
        for each in self.unitList:
            if each.name == unitName: 
                print("MAIN UNIT")
                if isinstance(each, rangedUnit): #TODO: support other types later
                    allActions, allindexes = each.allPossibleActions(self.unitList) #TODO: set niaze bara actions?
                    print(allindexes)
                    print("=====\n")
            else:
                print("OTHER UNITs")
                othersActions, othersIndexes = each.allPossibleActions(self.unitList)
                dataset = np.array(create_dataset(self, each.name)).reshape(1, 8, 4, 4) #OR A DICT {R1: [], R0 : []}
                print(dataset)
                print(othersActions)
                print(othersIndexes)
                output = model.model.predict(dataset)
                p = output[0][0]
                # v = output[1][0][0]
                newP = np.array(self.mask_probability_vector(othersIndexes, p))
                newP = newP[othersIndexes]
                print(newP)

                action_selected = newP.tolist().index(np.random.choice(newP, p=newP))
                actionString = othersActions[action_selected]
                dictionary[each.name] = actionString
        ...
        for action in allActions:
            obj = self.make_move(action, unitName, dictionary) #do one acton + all the fixed actions
            # print(obj.hash())
            allObjs.append(obj)

        # print(dictionary)
        # input("++++++")
        return allObjs, allindexes
    ...
    def make_move(self, agentAction, unitName, dictionary): # TODO: remove duplicate states? (check every attribute)
        #TODO: this is for only 2 unit games. we have to extend it to other for than two unit ones later.

        # apply action for unitName + other indexes from dictionary for other units
        ...
        for eachU in tmp.unitList:
            ...
            if eachU.name == unitName:
                action = agentAction
                tmp.single_unit_action = agentAction
            elif eachU.name in dictionary:
                action = dictionary[eachU.name] 
            ...
            if action == "right" and eachU.y!=3:
                ...
                name = action[7:]
                targetUnit = tmp.returnUnitbyName(tmp.unitList, name)
                targetPrevUnit = self.returnUnitbyName(self.unitList, name)
                if eachU.canAttack(targetPrevUnit):
                    targetUnit.hitpoints -= 1
                if eachU.ownerID == 1:
                    tmp.p1_actions.append([action[:6], name])
                elif eachU.ownerID == 2:
                    tmp.p2_actions.append([action[:6], name])
            else:
                assert 8!=8
...
def evaluate(action, u_me, b):
    ...
    if action[0] == "attack":
        ...
        # print(action)
        if action == "right":
            if u_me.y+1<= 3 and b.board[(u_me.x*4) + (u_me.y+1)] == "  ":
                u_me.y += 1
            else: 
                print("COULN'T MOVE")
    
        elif action == "left":
            if u_me.y-1>=0 and b.board[(u_me.x*4) + (u_me.y-1)] == "  ":
                u_me.y -= 1
            else: 
                print("COULN'T MOVE")
        elif action == "up":
            if u_me.x-1>=0 and b.board[((u_me.x -1)*4) + (u_me.y)] == "  ":
                u_me.x -= 1
            else: 
                print("COULN'T MOVE")
        elif action == "down":
            if u_me.x+1<=3 and b.board[((u_me.x +1)*4) + (u_me.y)] == "  ":
                u_me.x += 1
            else: 
                print("COULN'T MOVE")
        elif action == "nothing":
...
def create_dataset(board, unitName):
    data = np.zeros((2, 4, 4))
...
def start(x1, y1, x2, y2,x3, y3, max_iterations, model, mainUnitName):
    b = Board()
    ...
    u2 = rangedUnit(x1, y1, "R2", 2, 3)
    u5 = rangedUnit(x2, y2, "R1", 1, 1)
    u6 = rangedUnit(x3, y3, "R0", 1, 1)
    ...
    while(gameOver==False):
        ...
        for u in b.unitList:
            ...
            if u.name==mainUnitName: # we should call first max and then min node
                print("PLAYER MAX")
                memory_states.append(b)

                for i in range(5000):
                    tree.do_rollout(b, model, mainUnitName) 
                print("AFTER ROLLLOUT")
                pi = tree.returnDist(b)  # for training, you can use randomly, (not max), but for competetive play, you need to take the best action
...
if __name__ == "__main__":
    ...
    # combinations = [(0, 1), (0, 2), (0, 15), (3, 12), (5, 10), ()]
    training = True
    if training:
        for itr in range(1):
            dataset = []
            ...
            for i in range(1):
                ...
                winner, states, pis, zs, max_flag = start(x1,y1,x2,y2,x3,y3, max_iterations, model, mainUnitName) # one self-play until terminal
                ...
                if max_flag == False:
                    ...
                    for each in states:
                        print(each.hash())
                        dataset.append(create_dataset(each, mainUnitName))
            ...
            if len(z_list) != 0: # this means that we have data pairs (states, pi, z)
                ...
                with open("equal_hit_model%s.json"%itr, "w") as json_file:
                    json_file.write(model_json)
                model.model.save_weights("equal_hit_model%s.h5"%itr)
    ...
    # itr = 5
    # json_file = open('CC/equal_hit_model%s.json'%itr, 'r')
    # loaded_model_json = json_file.read()
    # json_file.close()
    # loaded_model = model_from_json(loaded_model_json)
    # # load weights into new model
    # loaded_model.load_weights("CC/equal_hit_model%s.h5"%itr)
    # model = loaded_model
    ...
    #             out = model.model.predict(data.reshape(1,13,4,4)) # without hitpoints it wqas (1,3,4,4)
    #             P = out[0][0]
    #             v = out[1][0][0]
    #             print(P, v)
    #             print(v)
